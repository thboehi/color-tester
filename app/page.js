  ...
  return (
    <div className={`flex relative flex-col items-center justify-center min-h-screen ${bgColor}`}>
      ...
      <button 
        ...
        className="absolute top-5 right-5 z-50 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 backdrop-blur-sm border border-gray-400/20 hover:bg-white/20 transition-all"
      >
      ...
      <main className={`flex flex-col gap-6 items-center justify-center min-h-screen py-2 font-[family-name:var(--font-geist-mono)] transition-opacity duration-300 ${controlsVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="flex flex-col gap-4 items-center mt-6">
          ...
          <div className="flex items-center gap-4">
            <label htmlFor="speed-control" className="text-gray-500">Vitesse:</label>
            <input
              id="speed-control"
              type="range"
              min="20"
        ...
        <div className="mt-4">
          <p className={`text-xs ${wakeLockActive ? 'text-green-500' : 'text-gray-500'}`}>
            {wakeLockActive 
              ? '✓ Mode veille désactivé' 
              : '✗ Mode veille activé (non supporté par ce navigateur)'}
          </p>
        </div>
      </main>
      
      <footer className="absolute bottom-0 left-0 h-24 w-full flex justify-center items-center z-50 font-[family-name:var(--font-geist-mono)]">
        <p className={`text-gray-500 transition-opacity duration-300 ${controlsVisible ? 'opacity-100' : 'opacity-10'} p-6 -m-6`}>
          This app was made for my Bachelor to test the energy consumption of differents types of screen.
        </p>
      </footer>
    </div>
  );
